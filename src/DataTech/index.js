...
const { Search } = Input
class DataTech extends Component{
  state = {
    xData:[],
    yData:[],
    ids:[],
  ...
  getTotal = (search)=>{
    wyAxiosPost('Technology/getTechnologyCount',{search},(result)=>{
  ...
  getParkList = (pagination=undefined, filters=undefined, sorter=undefined)=>{
    const doRequest = ()=>{
      ...
      wyAxiosPost('Technology/getTechnologyList',{current,pageSize,search},(result)=>{
        const responseData = result.data.msg
  ...
  delPark = ()=>{
    ...
    wyAxiosPost('Technology/delTechnology',{ids},(result)=>{
      const responseData = result.data.msg
  ...
  editPark = (id)=>{
    if(this._isMounted){
      this.setState({
        ...
        wyAxiosPost('Technology/getTechnologyById',{id: id},(result)=>{
          const responseData = result.data.msg
          if(this._isMounted){
            this.setState({
  ...
  render(){
    ...
    return (
      <div>
          ...
          <Col>
            <div css={{display:"flex"}}>
              ...
              <div css={{flex:"0 0 240px"}}>
                <Button onClick={this.addPark} type="primary"><i className="fa fa-plus-square-o" aria-hidden="true"></i> <span css={{marginLeft:"5px"}}>添加科技创新</span></Button>
        ...
        <Drawer
          title={this.state.id === ''?'新增科技创新':'修改科技创新'}
          width={"90%"}
