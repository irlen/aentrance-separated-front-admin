...
class DataNote extends Component{
  ...
  getTotal = (search)=>{
    wyAxiosPost('Park/getParkCount',{search},(result)=>{
  ...
  //删除
  delPark = ()=>{
    const ids = _.cloneDeep(this.state.ids)
    wyAxiosPost('Park/delPark',{ids},(result)=>{
      const responseData = result.data.msg
  ...
  editPark = (id)=>{
    if(this._isMounted){
      this.setState({
        ...
        wyAxiosPost('Park/getParkByName',{name: id},(result)=>{
          const responseData = result.data.msg
          if(this._isMounted){
            this.setState({
  ...
  render(){
    ...
    return (
      <div>
          ...
          <Col>
            <div css={{display:"flex"}}>
              ...
              <div css={{flex:"0 0 240px"}}>
                <Button onClick={this.addPark} type="primary"><i className="fa fa-plus-square-o" aria-hidden="true"></i> <span css={{marginLeft:"5px"}}>添加产业园</span></Button>
        ...
        <Drawer
          title={this.state.id === ''?'新增产业园':'修改产业园'}
          width={"90%"}
